...
class UploadSliderComponent extends Component
{
    private $max_files = 1;
    ...
    private function sendFile($data, $idBanner = null)
    {
        ...
        if (!in_array($this->file_ext, $this->type_allowed)) {
            ...
            return $this->_registry->getController()->redirect(['controller' => 'storesSliders', 'action' => 'add']);
        }

        $this->_registry->getController()->Flash->success(__('As imagens foram salvas com sucesso.'));

        return $this->_registry->getController()->redirect(['controller' => 'storesSliders', 'action' => 'index']);
    }
}
