<section class="single_item">
    <div class="container">
                ...
                <div class="single_item-details">
                    ...
                    <?php foreach ($storesColors as $storesColor) : ?>
                        <?php if ($storesColor->color2 && $storesColor->color3) : ?>
                            <a href="<?= $this->request->base ?>/homes/product-view/<?= $storesColor->stores_products_id ?>/<?= $storesColor->id ?>/<?= $storesColor->product_flag_code ?>" class="btn"
                                ...
                                background-image: linear-gradient(<?= $storesColor->color ?>, <?= $storesColor->color2 ?>, <?= $storesColor->color3 ?>); !important; border-radius: 10px; border: 1px solid #d7d7d7"></a>
                        <?php elseif ($storesColor->color && $storesColor->color2) : ?>
                            <a href="<?= $this->request->base ?>/homes/product-view/<?= $storesColor->stores_products_id ?>/<?= $storesColor->id ?>/<?= $storesColor->product_flag_code ?>" class="btn"
                                style="background-color: <?= $storesColor->color ?>;
                                background-image: linear-gradient(<?= $storesColor->color ?>, <?= $storesColor->color2 ?>); !important; border-radius: 10px; border: 1px solid #d7d7d7"></a>
                        <?php else : ?>
                            <a href="<?= $this->request->base ?>/homes/product-view/<?= $storesColor->stores_products_id ?>/<?= $storesColor->id ?>/<?= $storesColor->product_flag_code ?>" class="btn"
                    ...
                    <form action="<?= $this->request->base; ?>/stores-carts/add" method="post">
                        ...
                        <button class="btn btn-primary btn-default" type="submit">
                            <i class="fa fa-cart-shopping"></i> Adicionar ao carrinho
                        </button>
        ...
        <div class="row">
            <form method="post" action="<?= $this->request->base ?>/storesComments/add/<?= $storesProduct->id ?>">
                <h4>Adicionar Comentário</h4>
                <br>
                <div class="row">
                    <div class="form-group col-lg-12">
                        <input type="text" class="form-control" name="name_user" id="exampleInputName" placeholder="Nome*" required>
                    </div>
                    <div class="form-group col-md-12">
                        ...
                        <input
                            ...
                            name="_csrfToken"
                            value="<?= $this->request->getParam('_csrfToken'); ?>" />
                        <textarea class="form-control" id="exampleTextarea" name="comment" rows="3" placeholder="Mensagem*" required></textarea>
                    </div>
                </div>
        ...
        <ul class="row">
            <?php if (!empty($storesComments->toArray())) : ?>
                <?php foreach ($storesComments as $storesComment) : ?>
                    <li class="col-md-6">
                        <div class="media deal_single">
                            <div class="media-body">
                                <a href="store-cart.html">
                                    <h3 class="mt-0"><?= $storesComment->name_user ?></h3>
                                </a>
                                <p>
